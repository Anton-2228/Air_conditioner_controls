...
class Socket:
    ...
    def turn_on(self, minutes: int = 0) -> None:
        ...
        minutes = 0 значит «включить и оставить», поэтому отсчёт явно
        обнуляется: иначе включение поверх заведённого таймера оставило бы
        его тикать, и бойлер позже выключился бы сам.
        """
        self._send([(DP_SWITCH, True), (DP_COUNTDOWN, minutes * 60)])
